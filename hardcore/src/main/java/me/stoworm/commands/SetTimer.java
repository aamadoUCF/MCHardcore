...
public class SetTimer implements CommandExecutor
{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String [] args)
    {
        ...
        if (label.equalsIgnoreCase("setTimer"))
        {
            ...
            if (args.length <= 0)
            {
                sender.sendMessage(ChatUtils.prefix + ChatColor.RED + "Incorrect arguments. Correct usage: /setTimer <timeInMinutes>");
                return false;
            }
            ...
            if (!(plugin.configUtils.setConfigValue(timeInput, "timer.length")))
            {
                sender.sendMessage(ChatUtils.prefix + ChatColor.RED + "Please enter a number.");
                return false;
            }

            sender.sendMessage(ChatUtils.prefix + ChatColor.GREEN + "Sucess! " + ChatColor.GRAY + "Timer length set to " + ChatColor.GREEN +
